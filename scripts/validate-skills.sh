...
if rg -n '\bmente\.casa\b|\bname:\s*mente-docs\b' "$ROOT_DIR/projects" "$ROOT_DIR/opencode" >/dev/null 2>&1; then
  echo "ERROR: quedan referencias obsoletas a mente/mente.casa. Usa atlas/atlas.casa." >&2
  rg -n '\bmente\.casa\b|\bname:\s*mente-docs\b' "$ROOT_DIR/projects" "$ROOT_DIR/opencode" >&2
  exit 1
fi

if rg -n 'skills sugeridas si existen: .*sdd-proposal|skill \*\*`sdd-proposal`' \
  "$ROOT_DIR/opencode" "$ROOT_DIR/projects" \
  --glob '!projects/workflow/skills/common/sdd-proposal/SKILL.md' >/dev/null 2>&1; then
  ...
  rg -n 'skills sugeridas si existen: .*sdd-proposal|skill \*\*`sdd-proposal`' \
    "$ROOT_DIR/opencode" "$ROOT_DIR/projects" \
    --glob '!projects/workflow/skills/common/sdd-proposal/SKILL.md' >&2
  exit 1
fi
