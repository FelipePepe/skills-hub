...
while IFS=$'\t' read -r app_id install_path detect_csv sources_csv; do
  ...
  if [[ "$detected" == false && "$INCLUDE_MISSING" == false ]]; then
    echo "SKIP: $app_id agents (app no detectada)"
    continue
  fi
  ...
  for source_dir in "${source_list[@]}"; do
    [[ -n "$source_dir" ]] || continue
    abs_source="$ROOT_DIR/$source_dir"
    [[ -d "$abs_source" ]] || { echo "ERROR: agent source inexistente -> $abs_source"; errors=1; continue; }

    while IFS= read -r -d '' agent_file; do
      agent_name="$(basename "$agent_file")"
      [[ "$agent_name" == _* || "$agent_name" == .* ]] && continue
      dst="$install_path/$agent_name"

      if [[ -L "$dst" ]]; then
        echo "DRIFT: $app_id agents/$agent_name es un symlink (debe ser copia) -> $(readlink "$dst")"
        errors=1
        continue
      fi
      if [[ ! -f "$dst" ]]; then
        echo "DRIFT: $app_id agents/$agent_name falta en destino ($dst)"
        errors=1
        continue
      fi

      tmp_out="$(mktemp)"
      if ! rsync -ani "$agent_file" "$dst" > "$tmp_out" 2>/dev/null; then
        echo "ERROR: fallo rsync al comparar $agent_file con $dst"
        errors=1
      elif [[ -s "$tmp_out" ]]; then
        echo "DRIFT: $app_id agents/$agent_name"
        sed 's/^/    /' "$tmp_out"
        errors=1
      fi
      rm -f "$tmp_out"
