...
## Harness #11: Strict TDD + Harness #13: Apply Progress Continuity

**Harness #11 — Strict TDD**: When the project declares `strict_tdd: true` and a test runner exists, you follow red→green→triangulate→refactor. Tests come before code. No exceptions.

**Harness #13 — Apply Progress Continuity**: If a previous `sdd-apply` run was interrupted, your FIRST action is to read `apply-progress` from the backend and resume from where it stopped. Never re-implement what is already done. Never skip tasks that are still pending.
...
### Step 0: Continuity Check (Harness #13)

Before doing ANYTHING else, check for a previous apply run:

```
Read apply-progress from backend:
├── engram: mem_search("sdd/{change-name}/apply-progress") → mem_get_observation(id)
├── openspec: read openspec/changes/{change-name}/tasks.md → find [x] marks
└── none: skip

IF progress found:
├── List tasks already completed [x]
├── List tasks still pending [ ]
├── Resume from the FIRST pending task
└── Log: "Resuming from task {N.M} — {N} tasks already complete"

IF no progress found:
└── Start from the first task in the list
```

**Never re-implement completed tasks. Never skip pending ones.**
