...
### Step 5: Delivery Risk Assessment (Harness #18)

After persisting tasks, include a preliminary delivery risk estimate in the return envelope so the orchestrator can invoke `review-warlock` if needed:

```
ESTIMATE:
├── Total tasks: {N}
├── Estimated lines of code: {rough estimate}
├── Independent areas touched: {count areas from design.file-changes}
├── Database migrations: {yes/no}
├── Public API changes: {yes/no}
└── Risk signal: LOW | MODERATE | HIGH
```

The orchestrator uses this to decide whether to invoke `review-warlock` before `sdd-apply`.
...
**Signals**: {list any risk signals found}
**Recommendation**: Single PR | Invoke review-warlock

### Next Step
{If LOW risk: Ready for implementation (sdd-apply)}
{If MODERATE/HIGH risk: Recommend running review-warlock before sdd-apply}
```
