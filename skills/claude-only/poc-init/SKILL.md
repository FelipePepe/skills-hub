...
description: >
  ...
  structure ready for deploy-casa. Trigger: "create a new poc", "new project",
...
- A frontend+backend base is needed without reinventing the structure.
...
1. Confirm project name, desired `.casa` domain, and whether auth is needed.
2. Create repo with structure:
...
5. Add OpenAPI, minimal tests, and dev/build/test scripts.
6. Verify backend and frontend start correctly before marking as done.
...
- Maintain TypeScript strict in both backend and frontend.
- Use functional Express controllers, not classes, unless the project requires otherwise.
- Every PoC must ship with minimal tests and a working `start-dev.sh`.
- If the user wants something smaller than the standard stack, reduce scope explicitly.

## Output contract

```
INIT:{project} DOMAIN:{name.casa} STACK:{angular+express|reduced} GITFLOW:{ok} NEXT:{sdd-new|manual}
```
